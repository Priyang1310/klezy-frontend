...
const NewCard = ({ post }) => {
  console.log(post);
	...
	// Format work basis
	const jobType =
		Object.keys(workBasis)
			.filter((key) => workBasis[key])
			.join(", ") || "Not specified";

	// Format work mode and location
	const workModeString =
		Object.keys(workMode)
			.filter((key) => workMode[key])
			.join(", ") || "";
	const locationString = [district, state, country]
		.filter(Boolean)
		.join(", ");
				...
				<div className="flex items-center justify-between w-full mt-2">
					...
					<div className="flex justify-end gap-2 mt-1 w-full">
						<div className="flex gap-2 items-center">
							<button
								className="text-[#A100FF] text-xl p-1 rounded-full hover:bg-purple-50 transition-colors"
								onClick={() => setIsModalOpen(true)}
							>
						...
						<button
							className=" text-purple-600 hover:bg-purple-100 transition-all duration-300 text-xl font-medium px-2 py-2 rounded-full"
							onClick={() => {
