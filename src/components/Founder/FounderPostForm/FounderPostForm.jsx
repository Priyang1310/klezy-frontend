...
function FounderPostForm({ onClose }) {
	...
	const validateStep1 = () => {
		...
		if (!formData.profile_pic)
			newErrors.profile_pic = "Profile Pic is required";
	...
	const handleNext = async () => {
		...
		if (step === 1) {
			...
			textFields = [
				...
				{
					field: "timeCommitment.value",
					value: formData.timeCommitment.value,
				},
				{
					field: "timeCommitment.unit",
		...
		if (validateStep3()) {
			try {
				...
				const countryObj = countries.find(
					(c) => c.isoCode === formData.workLocation.country
				);
				...
				const stateObj = states.find(
					(s) => s.isoCode === formData.workLocation.state
				);
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div className="relative">
								...
								{errors.timeCommitment && (
									<div className="text-red-500 text-xs sm:text-sm mt-1">
										...
										{errors.timeCommitment.unit && (
											<p>{errors.timeCommitment.unit}</p>
										)}
									</div>
								)}
							</div>
							...
							<div className="relative">
								...
								{errors.experienceRange && (
									<div className="text-red-500 text-xs sm:text-sm mt-1">
										{errors.experienceRange.min && (
											<p>{errors.experienceRange.min}</p>
										)}
										{errors.experienceRange.max && (
											<p>{errors.experienceRange.max}</p>
										)}
									</div>
								)}
							</div>
							...
							<div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
								<div>
									...
									<select
										...
										className={`w-full h-12 px-3 text-gray-500 sm:px-4 py-2 sm:py-3 border rounded-lg focus:ring-2 focus:ring-purple-200 transition-all duration-200 hover:border-purple-400 text-sm sm:text-base ${
